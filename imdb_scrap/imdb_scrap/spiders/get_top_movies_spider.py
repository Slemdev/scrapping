...
class GetTopMoviesSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        duration_str = response.css(".ipc-inline-list__item::text").extract_first()
        # Séparer et convertir les heures et les minutes en int(minutes)
        hours, minutes = duration_str.split('h')
        hours = int(hours)
        minutes = int(minutes[:-1])
        items['duration'] = hours * 60 + minutes
...
class GetTopSeriesSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        items['rating'] = response.css(".sc-bde20123-1.iZlgcd::text").extract_first()
        items['genre'] = response.css(".ipc-chip__text::text").extract()
        duration_str = response.xpath("(//div[@class='sc-52d569c6-0 kNzJA-D']//li)[4]/text()").extract_first()
        # Séparer et convertir les heures et les minutes en int(minutes)
        hours, minutes = duration_str.split('h')
        hours = int(hours)
        minutes = int(minutes[:-1])
